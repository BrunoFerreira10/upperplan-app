...
function update080xto0830()
{
   ...
   // Problems management
    if (!$DB->tableExists('glpi_problems')) {
        ...
        $DB->doQueryOrDie($query, "0.83 create glpi_problems");
    ...
    if (!$DB->tableExists('glpi_problems_users')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_problems_users");
    }
    ...
    if (!$DB->tableExists('glpi_groups_problems')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_groups_problems");
    }
    ...
    if (!$DB->tableExists('glpi_items_problems')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_items_problems");
    }
    ...
    if (!$DB->tableExists('glpi_problems_tickets')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_problems_tickets");
    }
    ...
    if (!$DB->tableExists('glpi_problemtasks')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_problemtasks");
    }
   ...
   // Merge tickettasks and ticket planning
       ...
       // migrate DATA
        ...
        foreach ($DB->request('glpi_ticketplannings') as $data) {
            if ($task->getFromDB($data['tickettasks_id'])) {
                ...
                $DB->doQueryOrDie($query, "0.83 migrate planning to glpi_tickettasks");
            }
        }
    ...
    if ($result = $DB->doQuery($query)) {
        if ($DB->numrows($result) == 0) {
            ...
            $DB->doQueryOrDie($query, "0.83 add problem notification");
            $notid = $DB->insertId();
...
&lt;p&gt;##ENDFOREACHtasks##&lt;/p&gt;
&lt;/div&gt;')";
            $DB->doQueryOrDie($query, "0.83 add problem notification translation");
   ...
   // Clean `glpi_networkports_vlans` datas (`networkports_id` whithout networkports)
    ...
    $DB->doQueryOrDie($query, "0.83 clean networkports_vlans datas");
    ...
    foreach ($itemtype_tables as $table) {
        foreach ($typestochange as $key => $val) {
            $query = "UPDATE `$table`
                   SET `itemtype` = '$val'
                   WHERE `itemtype` = '$key'";
            $DB->doQueryOrDie($query, "0.83 update itemtype of table $table for $val");
        }
    }
    ...
    foreach ($types as $ext => $data) {
        ...
        if ($result = $DB->doQuery($query)) {
            if ($DB->numrows($result) == 0) {
                ...
                $DB->doQueryOrDie($query, "0.83 add document type $ext");
            }
        }
    }
   /// Update icons
    ...
    foreach ($types as $ext => $icon) {
        ...
        if ($result = $DB->doQuery($query)) {
            if ($DB->numrows($result) == 1) {
                ...
                $DB->doQueryOrDie($query, "0.83 update icon for doc type $ext");
            }
        }
    }
            ...
            if ($DB->numrows($result) > 0) {
                while ($data = $DB->fetchAssoc($result)) {
                   ...
                   // manage is_dynamic :
                    ...
                    $DB->doQueryOrDie($query2, "0.83 move emails to  glpi_useremails");
                }
            }
        }
   ...
   // check unicity for users email : unset rule and display warning
    foreach (
        $DB->request(
            "glpi_fieldunicities",
            "`itemtype` = 'User' AND `fields` LIKE '%email%'"
        ) as $data
    ) {
   ...
   /// migration : multiple group managers
                            ...
                            if ($DB->numrows($result2) == 0) {
                              ...
                              // Update user as manager if presnet in groups_users
                                $query2 = "UPDATE `glpi_groups_users`
                                   ...
                                   WHERE `groups_id` = '" . $data['id'] . "'
                                         AND `users_id` = '" . $data['users_id'] . "'";
                                $DB->doQueryOrDie($query2, "0.83 set manager of groups");
                            }
                        }
                    }
                }
        ...
        if ($result = $DB->doQuery($query)) {
            if ($DB->numrows($result) > 0) {
                ...
                foreach ($tab as $old => $new) {
                    ...
                    $DB->doQueryOrDie($query, "0.83 update datas for rules actions");
                }
                // Update criteria
                foreach ($tab as $old => $new) {
                    $query = "UPDATE `glpi_rulecriterias`
                         ...
                         WHERE `criteria` = '$old'
                               AND `rules_id` IN ($rules)";
                    $DB->doQueryOrDie($query, "0.83 update datas for rules criteria");
                }
            }
        }
    }
    ...
    if (!$DB->tableExists('glpi_tickettemplates')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_tickettemplates");

        $query = "INSERT INTO `glpi_tickettemplates`
                       (`name`, `is_recursive`)
                VALUES ('Default', 1)";
        $DB->doQueryOrDie($query, "0.83 add default ticket template");
    ...
    if (!$DB->tableExists('glpi_tickettemplatehiddenfields')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_tickettemplatehiddenfields");
    }
    ...
    if (!$DB->tableExists('glpi_tickettemplatepredefinedfields')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_tickettemplatepredefinedfields");
    }
        ...
        if ($default_ticket_template > 0) {
            foreach ($DB->request('glpi_configs') as $data) {
                if (isset($data['is_ticket_title_mandatory']) && $data['is_ticket_title_mandatory']) {
                    ...
                    $DB->doQueryOrDie($query, "0.83 add mandatory field for default ticket template");
                }
   ...
   // Clean unused slalevels
    $query = "DELETE
             ...
             WHERE (`glpi_slalevels_tickets`.`tickets_id`, `glpi_slalevels_tickets`.`slalevels_id`)
                  NOT IN (SELECT `glpi_tickets`.`id`, `glpi_tickets`.`slalevels_id`
                          FROM `glpi_tickets`)";
    $DB->doQueryOrDie($query, "0.83 clean glpi_slalevels_tickets");
    ...
    if (!$DB->tableExists('glpi_ticketrecurrents')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_ticketrecurrents");
    ...
    if (!countElementsInTable('glpi_crontasks', ['itemtype' => 'TicketRecurrent', 'name' => 'ticketrecurrent'])) {
        ...
        $DB->doQueryOrDie($query, "0.83 populate glpi_crontasks for ticketrecurrent");
    }
   ...
   // New index for count on tab
    ...
    foreach ($profiles as $profile => $data) {
        $query  = "INSERT INTO `glpi_profiles`
                  ...
                  VALUES ('" . implode("', '", $data) . "')";
        $DB->doQueryOrDie($query, "0.83 create new profile $profile");
    }
    ...
    if (!$DB->tableExists('glpi_reminders_users')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_reminders_users");
    }
    ...
    if (!$DB->tableExists('glpi_groups_reminders')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_groups_reminders");
    }
    ...
    if (!$DB->tableExists('glpi_profiles_reminders')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_profiles_reminders");
    }
        ...
        if ($result = $DB->doQuery($query)) {
            if ($DB->numrows($result) > 0) {
                ...
                foreach (
                    $DB->request(
                        "glpi_profiles",
                        "`interface` = 'helpdesk' AND `reminder_public` = 'r'"
                    ) as $data2
                ) {
   ...
   // Migrate datas for entities + drop fields : is_private / entities_id / is_recursive
                ...
                while ($data = $DB->fetchAssoc($result)) {
                    $query = "INSERT INTO `glpi_entities_reminders`
                         ...
                         VALUES ('" . $data['id'] . "', '" . $data['entities_id'] . "',
                                 '" . $data['is_recursive'] . "');";
                    $DB->doQueryOrDie($query, "0.83 migrate data for public reminders");
                }
            }
        }
    ...
    if (!$DB->tableExists('glpi_knowbaseitems_users')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_knowbaseitems_users");
    }
    ...
    if (!$DB->tableExists('glpi_groups_knowbaseitems')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_groups_knowbaseitems");
    }
    ...
    if (!$DB->tableExists('glpi_knowbaseitems_profiles')) {
        ...
        $DB->doQueryOrDie($query, "0.83 add table glpi_knowbaseitems_profiles");
    }
            ...
            if ($DB->numrows($result) > 0) {
                while ($data = $DB->fetchAssoc($result)) {
                    $query = "INSERT INTO `glpi_entities_knowbaseitems`
                         ...
                         VALUES ('" . $data['id'] . "', '" . $data['entities_id'] . "',
                                 '" . $data['is_recursive'] . "');";
                    $DB->doQueryOrDie($query, "0.83 migrate data for entities on glpi_entities_knowbaseitems");
                }
            }
        }
   ...
   // create root entity if not exist with old default values
    if (countElementsInTable('glpi_entitydatas', ['entities_id' => 0]) == 0) {
        ...
        $DB->doQueryOrDie($query, "0.83 add entities_id 0 in glpi_entitydatas");
        ...
        $DB->doQueryOrDie($query, "0.83 update tickettemplates_id for root entity in glpi_entitydatas");
        ...
        $query = "UPDATE `glpi_entitydatas`
                ...
                WHERE `entities_id` = 0
                      AND `tickettype` = 0";
        $DB->doQueryOrDie($query, "0.83 update tickettype for root entity in glpi_entitydatas");

        $query = "UPDATE `glpi_entitydatas`
                SET `inquest_config` = 1
                WHERE `entities_id` = 0
                      AND `inquest_config` = 0";
        $DB->doQueryOrDie($query, "0.83 update inquest_config for root entity in glpi_entitydatas ");

        $query = "UPDATE `glpi_entitydatas`
                SET `inquest_rate` = 0
                WHERE `entities_id` = 0
                      AND `inquest_rate` = '-1'";
        $DB->doQueryOrDie($query, "0.83 update inquest_rate for root entity in glpi_entitydatas ");
    ...
    foreach ($field0 as $field_0) {
        if ($DB->fieldExists("glpi_entitydatas", $field_0, false)) {
            $query = "UPDATE `glpi_entitydatas`
                   ...
                   WHERE `$field_0` = '0'
                         AND `entities_id` > 0";
            $DB->doQueryOrDie($query, "0.83 new value for inherit parent 0 in glpi_entitydatas");
        }
    }
            ...
            if ($data = $DB->fetchAssoc($result)) {
                foreach ($fieldconfig as $field_config) {
                    if (
                        ...
                        $DB->doQueryOrDie($query, "0.83 migrate data from config to glpi_entitydatas");
                ...
                if ($DB->fieldExists("glpi_entitydatas", "auto_assign_mode", false)) {
                   // new value for never
                    ...
                    $DB->doQueryOrDie($query, "0.83 change value Never in glpi_entitydatas for auto_assign_mode");
                }
            }
        }
    }
   ...
   // value of config in each entity
    ...
    if ($restore_root_entity_value) {
        $query = "UPDATE `glpi_entitydatas`
                SET `calendars_id` = 0
                WHERE `entities_id` = 0;";
        $DB->doQueryOrDie($query, "0.83 restore root entity default value");
    }
